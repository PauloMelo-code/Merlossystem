...
| `src/lib/integracoes/bling/leitura.ts` | `listarProdutos`, `detalharProduto`, `lerSaldos` com Zod tolerante |
...
`produto_preco_alterado`, o resto `produto_sincronizado`, com ator `sistema`.
Produto que some do Bling não é excluído. Ao terminar, carimba
`lojas_integracoes.ultima_sincronizacao` (ou `ultimo_erro`).
...
devolvido não muda mais. Rastreio aceita só link `https://`.
...
- **Ator de sistema**: não existe `ATOR_SISTEMA` semeado. A sincronização grava
  com `autorId` nulo e `origem: "worker"`: `modified_by` fica nulo (a FK aceita)
  e a trilha registra `ator_tipo = 'sistema'` sem `ator_id` (o CHECK aceita).
- **Trilha "antes do efeito"** em cancelar e dispensar: a gravação passa por
  `atualizarComTrava()`, que registra a trilha na MESMA transação, com o motivo
  no diff (`cancelado_motivo`, `masc_observacao`); falha da trilha desfaz o
  efeito.
- **Agendamento**: `src/lib/fila/agendamentos.ts` não agenda
  `sincronizar-bling`; hoje o job só roda quando alguém o enfileira.
...
| `tests/unidade/catalogo-cliente-bling.test.ts` | só GET, balde da conta antes de sair, erro permanente × temporário |
| `tests/integracao/pedidos-fluxo.test.ts` | fechar venda, negócio → ganho, CHECKs `pedidos_total_coerente` e `itens_total_coerente`, Masc, cancelamento, escopo, índice parcial da fila |
| `tests/integracao/catalogo-disponibilidade.test.ts` | fórmula no banco, loja sem depósito, SKU de outra loja |
| `tests/integracao/catalogo-sincronizacao.test.ts` | criação, grade, idempotência, `produto_preco_alterado`, conta errada |
