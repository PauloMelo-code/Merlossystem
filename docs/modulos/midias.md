...
| `src/lib/midias/` | domínio: `upload.ts`, `edicao.ts` (pasta e etiquetas), `ingestao.ts` e `leitura.ts` (costuras de M1), `limpeza.ts` (chamada por M8), `_consultas.ts`, `dto.ts`, `index.ts` |
| `src/lib/actions/midias.ts` | `listarGaleria` (`midia:ler`), `editarMidia` (`midia:editar`), `excluirMidia` (`midia:excluir`, exclusão lógica) |
| `src/lib/validadores/midias.ts` | filtros da grade, parâmetros do upload, edição, exclusão |
| `src/app/api/midias/route.ts` | `POST` upload |
| `src/app/api/midias/[id]/route.ts` | `GET`/`HEAD` binário (`?miniatura=1`) |
| `src/app/(app)/galeria/` | tela: `area-upload.tsx`, `grade-midias.tsx`, `visualizador.tsx`, `organizar-midia.tsx` |
...
- **Organizar** (pasta e etiquetas, no visualizador): `midia:editar` (gestão e
  vendedor, este só na própria loja). Não é ação crítica (fora de
  `04-ui.md §9.1`), então grava direto, sem block. A pasta passa pela trava de
  colisão e só existe para `origem = 'upload'` (recebida com pasta =
  `VALIDACAO`). Etiquetas são ligação pura: o formulário manda o conjunto
  inteiro; o que saiu vira exclusão lógica, o que entrou nasce, e só etiqueta
  viva **da loja da mídia** vira vínculo (outra loja = `NAO_ENCONTRADO`). Tudo
  entra na trilha como `midia_alterada`; a action devolve o `updated_at` novo.
  Quem não tem `midia:editar` vê só os nomes das etiquetas.
...
- `guardarMidiaRecebida(tx, { lojaId, origem, bytes, tipoMime? }, ctx)` —
  ...
  (`origem = 'recebida'`, sem pasta, trilha `midia_recebida`) na transação de
  quem chama e devolve
...
- Worker e ingestão gravam com `contextoDeSistema({ origem: "worker", lojaId })`
  (`@/lib/db/mutacoes`): `modified_by` e `ator_id` = `ATOR_SISTEMA`,
  `ator_tipo = 'sistema'`. O `UPDATE` do anexo é `atualizarEstado` (lista
  `ESTADOS_DE_SISTEMA`: `midia_id`, `baixada`, `url_externa`), sem trilha.

## Costura com M1 (`src/lib/midias/leitura.ts`)

- `lerBinarioDaMidia(lojaId, midiaId): Promise<BinarioDaMidia>` — bytes,
  `mime`, `nomeOriginal` e `tamanhoBytes` de uma mídia **viva** da loja,
  para o adaptador subir ao provedor. Outra loja, excluída ou sem objeto no
  bucket: `ErroDeEscopo` (404). A exceção RN-M06 é só da rota de leitura.
...
| `tests/integracao/midias-rotas.test.ts` | 201/200 duplicada, 6 MB → 413, `.svg` → 415, extensão trocada → 415, escopo (outra loja 404, gestão lê a rede), `FALTA_LOJA`, viewer 403, `Range`, `HEAD`, `attachment`, organizar (pasta com trava, etiquetas com exclusão lógica, trilha `midia_alterada`, etiqueta de outra loja, viewer, outra loja, recebida sem pasta), exclusão lógica, RN-M06 (referenciada 200, não referenciada 404), colisão |
| `tests/integracao/midias-ingestao.test.ts` | ingestão por bytes com `ATOR_SISTEMA` e `midia_recebida`, job de miniatura, dedupe, recusa de `http://127.0.0.1:9002`, `169.254.169.254` e host fora da allowlist, sucesso do `baixar-de-url`, `lerBinarioDaMidia` (bytes, outra loja, sem objeto, excluída), limpeza de 90 dias, remoção LGPD, cursor nos dois sentidos |
| `tests/componentes/midias-galeria.test.tsx` | grade só com `/api/midias/…`, block de 3 s na exclusão, erro mantém o modal, organizar (só leitura sem permissão, conjunto inteiro de etiquetas da loja, sem pasta para recebida, erro na tela), envio por XHR |
...
- O job `limpar-midia` não marca "binário já removido": a varredura dos 90
  dias repete as mesmas linhas (remoção idempotente e barata). A marca entra
  por migração quando o volume pesar.
- A galeria não filtra por etiqueta (a especificação não pede); o filtro entra
  quando a loja tiver etiquetas de mídia em uso.
